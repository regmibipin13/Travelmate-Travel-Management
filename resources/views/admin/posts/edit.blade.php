    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.posts.update", [$post->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('post_category') ? 'is-invalid' : '' }}" name="post_category_id" id="post_category_id" required>
                    @foreach($post_categories as $id => $entry)
                        <option value="{{ $id }}" {{ (old('post_category_id') ? old('post_category_id') : $post->post_category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                <div class="form-check {{ $errors->has('enabled') ? 'is-invalid' : '' }}">
                    ...
                    <input class="form-check-input" type="checkbox" name="enabled" id="enabled" value="1" {{ $post->enabled || old('enabled', 0) === 1 ? 'checked' : '' }}>
