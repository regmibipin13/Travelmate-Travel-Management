...
@can('post_category_create')
    <div style="margin-bottom: 10px;" class="row">
        <div class="col-lg-12">
            <a class="btn btn-success" href="{{ route('admin.post-categories.create') }}">
                        ...
                        <tr data-entry-id="{{ $postCategory->id }}">
                            ...
                            <td>
                                @can('post_category_show')
                                    <a class="btn btn-xs btn-primary" href="{{ route('admin.post-categories.show', $postCategory->id) }}">
                                        {{ trans('global.view') }}
                                    </a>
                                @endcan

                                @can('post_category_edit')
                                    <a class="btn btn-xs btn-info" href="{{ route('admin.post-categories.edit', $postCategory->id) }}">
                                        {{ trans('global.edit') }}
                                    </a>
                                @endcan

                                @can('post_category_delete')
                                    <form action="{{ route('admin.post-categories.destroy', $postCategory->id) }}" method="POST" onsubmit="return confirm('{{ trans('global.areYouSure') }}');" style="display: inline-block;">
...
@can('post_category_delete')
  ...
  let deleteButton = {
    ...
    url: "{{ route('admin.post-categories.massDestroy') }}",
